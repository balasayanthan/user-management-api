...
namespace Api.Controllers
{
    ...
    {
        [HttpGet]
        public async Task<ActionResult<PagedResult<GroupDto>>> List([FromQuery] int page = 1, [FromQuery] int pageSize = 50)
        {
            return  Ok(await mediator.Send(new ListGroupsQuery(page, pageSize)));
        }
        ...
        {
             return Ok(await mediator.Send(new GetGroupByIdQuery(id)));
        }
        ...
        {
            var id = await mediator.Send(new CreateGroupCommand(dto));
        ...
        {
            await mediator.Send(new UpdateGroupCommand(id, dto));
            return NoContent();
        }
